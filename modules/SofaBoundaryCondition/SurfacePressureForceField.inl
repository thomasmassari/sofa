...
void SurfacePressureForceField<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
    ...
    defaulttype::Vec4f color(0.f, 0.8f, 0.3f, 1.f);

    const DataTypes::CPos& min = DataTypes::getCPos(m_min.getValue());
    const DataTypes::CPos& max = DataTypes::getCPos(m_max.getValue());
    defaulttype::Vector3 pmin(min[0], min[1], min[2]);
    defaulttype::Vector3 pmax(max[0], max[1], max[2]);

    vparams->drawTool()->drawBoundingBox(pmin, pmax, 1.0, color);
    ...
    if (m_drawForceScale.getValue() && m_f.size()==x.size())
    {
        ...
        color = defaulttype::Vec4f(0,1,0.5,1);
        defaulttype::Vector3 p0, p1;

        for (unsigned int i=0; i<x.size(); i++)
        {
            p0 = DataTypes::getCPos(x[i]);
            p1 = DataTypes::getCPos(x[i]) + DataTypes::getDPos(m_f[i])*m_drawForceScale.getValue();
            positions.push_back(p0);
            positions.push_back(p1);
        }
        vparams->drawTool()->drawLines(positions, 1, color);
    }
